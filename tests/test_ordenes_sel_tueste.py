...
class OrdenesSelTuesteTest(TestCase):
    @classmethod
    def setUpTestData(cls):
        ...
        cls.cafe = InventarioCafe.objects.create(codigo='C-STUE-001')
        cls.ordenes = []
        for i in range(5):
    ...
    def test_editar_ordenes(self):
        for i, orden in enumerate(self.ordenes[:4]):
            orden.prioridad += 1
            orden.save()
            self.assertEqual(OrdenSeleccionTostado.objects.get(pk=orden.pk).prioridad, i+2)
